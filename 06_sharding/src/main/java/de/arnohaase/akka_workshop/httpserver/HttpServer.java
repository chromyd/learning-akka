...
import de.arnohaase.akka_workshop.person.ImmutableGetPerson;
import de.arnohaase.akka_workshop.person.ImmutablePersonData;
import de.arnohaase.akka_workshop.person.ImmutableUpdatePerson;
                    ...
                    () -> route (
                            pathPrefix("persons", () -> route (
                                    pathEnd(() -> post(() -> entity(Jackson.unmarshaller(om, ImmutablePersonData.class),
                                            msg -> completeOKWithFuture(PatternsCS.ask(persons, msg, 1000), Jackson.marshaller(om))))),
                                    path(uuidSegment(), personId -> route (
                                            get(() -> completeOKWithFuture (PatternsCS.ask(persons, ImmutableGetPerson.of(personId), 1000), Jackson.marshaller(om))),
